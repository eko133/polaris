...
bin_data = main.peak_normalize(bin_data,normalize='tic')
bin_data = bin_data.replace(np.nan,0)
pca_data = main.custom_ica(bin_data)
pca_data['cor']=pca_data.index
pca_data=pca_data.reset_index()
pca_data['x'] = pca_data.cor.str.extract(r'R00X(.*?)Y').astype(int)
pca_data['y'] = pca_data.cor.str.extract(r'Y(.*?)$').astype(int)
pc=dict()
for i in range(1,11):
    pc[i] = pca_data[['x','y',f'PC{i}']]
    plt.imshow(pc[i].pivot('x','y',f'PC{i}'),cmap='gray')
    plt.show()
